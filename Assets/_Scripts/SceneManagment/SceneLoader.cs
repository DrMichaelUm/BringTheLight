    ...
    'requires': [
      ...
      { SceneConfig.Type.world, none },
        ...
        void Awake()
        {
            ...
            if (!File.Exists(Application.persistentDataPath + @"\scene_config.json"))
            {
                File.WriteAllText(Application.persistentDataPath + @"\scene_config.json",  jsonText);
            }
                var config = File.ReadAllText(Application.persistentDataPath + @"\scene_config.json");
